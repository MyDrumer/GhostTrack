...
def phoneGW():
    User_phone = input(
        f"\n {Wh}Enter Nomor Telepon target {Gr}Ex [+6281xxxxxxxxx] {Wh}: {Gr}")  # INPUT NUMBER PHONE
...
def showIP():
    ...
    print(f"\n {Wh}[{Gr} + {Wh}] IP Adrress Kamu! : {Gr}{Show_IP}")
    print(f"\n {Wh}==============================================")

# Setelah fungsi lain (misalnya setelah TrackLu)
def weather_tracker():
    city = input(f"{Wh}\nEnter City Name: {Gr}")
    api_key = "04e4aa57a28a3e0f10633c4079e2099d"  # Ganti dengan API key dari OpenWeatherMap
    ...
    if response.status_code == 200:
        ...
        print(f"{Wh}\nWeather in {Gr}{city}{Wh}:")
        print(f"{Wh}Temperature: {Gr}{temp}°C")
        print(f"{Wh}Description: {Gr}{weather_desc}")
        print(f"{Wh}Humidity: {Gr}{humidity}%")
        print(f"{Wh}Wind Speed: {Gr}{wind_speed} m/s")
    else:
        print(f"{Re}City not found or API error. Please try again.{Wh}")
...
options = [
    ...
    },
    {
     ...
     {
    'num': 6,  # Nomor pilihan baru untuk Weather Tracker
    'text': 'Weather Tracker',
    'func': weather_tracker
},
    {
        'num': 0,
        'text': 'Exit',
...
def option():
    ...
    \____/_/ /_/\____/____/\__/     /_/ /_/   \__,_/\___/_/|_| 
              ...
              {Wh}[ + ]  CODE BY SamXploit  [ + ]
             [ + ] YG UBAH MUKANYA KEK KONTOL [ + ]
    """)
